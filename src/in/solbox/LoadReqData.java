...
public class LoadReqData extends AsyncTask<String, Void, ArrayList<Entry_req>> {

	Context myContext;
	RequestsFragment HF;
   String mUrl="";
	ListAdapter_nonclick mAdapter;
    LoadReqData(ListAdapter_nonclick m,Context hai,RequestsFragment h){
        mAdapter=m;
        myContext=hai;
        HF=h;
    }
    ...
    @Override
    protected ArrayList<Entry_req> doInBackground(String... urls) {

        return downloadUrl(urls[0]);
    }
    private ArrayList<Entry_req> downloadUrl(String myUrl) {
        // TODO Auto-generated method stub
    	mUrl=myUrl;
    	ArrayList<Entry_req> entries=new ArrayList<Entry_req>();
        if(hasActiveInternetConnection(myContext)==true) {
        	...
        	try{
        	        ...
        	        while ((line = reader.readLine()) != null) {
        	        	 if(line.compareTo("")!=0){
        	                Entry_req ent=new Entry_req();
        	                ent.line1=line.toString();
        	                line = reader.readLine();
        	                ent.date=line.toString();
        	                entries.add(ent);
        	                }
        	        }
        	        is.close();
    ...
    @Override
    protected void onPostExecute(ArrayList<Entry_req> entries) {
